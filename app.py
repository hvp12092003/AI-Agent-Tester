...
if "agent_state" not in st.session_state:
    st.session_state.agent_state = {
        "model_name": "", "goal": "", "url": "", "screenshot": None,
        "history": [], "last_thought": "Sẵn sàng khởi động...",
        "findings": [], "is_complete": False, "base_url": None,
        "dom_elements": None, "global_url_queue": [],
        "current_page_plan": [], "testing_url": None,
        "clicked_selectors_blacklist": [],
        "phase": "exploration",
        "security_steps": 0,
        "test_ui": True, "test_security": True
    }
...
def auto_clear_state():
    if not st.session_state.running and st.session_state.agent_state.get("is_complete"):
        st.session_state.agent_state = {
            "model_name": "", "goal": "", "url": "", "screenshot": None,
            "history": [], "last_thought": "Tự động làm mới khi đổi trang...",
            "findings": [], "is_complete": False, "base_url": None,
            "dom_elements": None, "global_url_queue": [],
            "current_page_plan": [], "testing_url": None,
            "phase": "exploration",
...
if clear_btn:
    ...
    st.session_state.agent_state = {
        "model_name": "", "goal": "", "url": "", "screenshot": None,
        "history": [], "last_thought": "Đã làm mới và dọn dẹp bộ nhớ tạm...",
        "findings": [], "is_complete": False, "base_url": None,
        "dom_elements": None, "global_url_queue": [],
        "current_page_plan": [], "testing_url": None,
        "phase": "profiling",
        "security_steps": 0,
        "test_ui": True, "test_security": True,
        "testing_path": None, "path_steps": [], "current_step_index": 0
    }
    st.rerun()

# --- LOGIC CHẠY AGENT ---
async def run_agent_async(url, goal, model, mode, test_ui=True, test_security=True, login_user=None, login_pass=None):
    from tools.report_tool import cleanup_reports
    cleanup_reports()
    print(f"DEBUG APP: test_ui={test_ui}, test_sec={test_security}, mode={mode}")
    app = create_graph()
    
    # Mặc định luôn bắt đầu từ bước Lập kế hoạch (Planning)
    starting_phase = "planning"
    
    initial_state = {
        "goal": goal, "url": url, "screenshot": None, "next_action": None,
        "history": [], "last_thought": "Đang kết nối trình duyệt...",
        "findings": [], "is_complete": False, "model_name": model,
        "mode": mode, "base_url": None, "dom_elements": None,
        "global_url_queue": [], "current_page_plan": [], "testing_url": None,
        "clicked_selectors_blacklist": [],
        "phase": "planning",
        "security_steps": 0,
        "test_ui": test_ui, "test_security": test_security,
        "login_user": login_user, "login_pass": login_pass, "logged_in": False,
        "login_steps": 0, "login_attempts": 0,
        "testing_path": None, "path_steps": [], "current_step_index": 0,
        "master_plan": [], "security_memory": []
    }
    # Initial phase override if not test_web
    if mode == "custom":
        if not test_ui and test_security:
            initial_state["phase"] = "security"
        else:
            initial_state["phase"] = "exploration"
    ...
    try:
        async for event in app.astream(initial_state):
            ...
            for node_name, state in event.items():
                # Gắn nhãn node vào lịch sử
                if "history" in state and state["history"]:
                    last_h = state["history"][-1]
                    if isinstance(last_h, str) and not last_h.startswith("["):
                        state["history"][-1] = f"[{node_name.upper()}] {last_h}"
        ...
        # Đảm bảo đóng BrowserManager instance liên kết với event loop này
        try:
...
if start_btn:
    if not user_prompt:
        ...
        st.session_state.agent_state = {
            "model_name": selected_model, "goal": test_goal, "url": target_url, "screenshot": None,
            "history": [], "last_thought": "Đang kết nối trình duyệt...",
            "findings": [], "is_complete": False, "base_url": None,
            "dom_elements": None, "global_url_queue": [],
            "current_page_plan": [], "testing_url": None,
            "clicked_selectors_blacklist": [],
            "phase": "planning" if is_web_test else "exploration",
            "security_steps": 0,
            "test_ui": test_ui_checked, "test_security": test_sec_checked,
            "login_user": user_val, "login_pass": pass_val,
            "logged_in": False, "login_steps": 0, "login_attempts": 0,
            "master_plan": []
        }
        ...
        def run_in_thread(url, goal, model, mode, do_ui, do_sec, u_val, p_val):
            asyncio.run(run_agent_async(url, goal, model, mode, do_ui, do_sec, u_val, p_val))
        
        # Create thread and attach context
        thread = threading.Thread(target=run_in_thread, args=(
            target_url, test_goal, selected_model, selected_mode, test_ui_checked, test_sec_checked,
            user_val, pass_val
        ))
...
with c1:
    ...
    if st.session_state.agent_state["screenshot"]:
        ...
        st.info("Chưa có hình ảnh. Nhấn 'CHẠY' để kích hoạt Agent.")

with c2:
    # Master Plan View
    st.markdown('<div class="glass-card">', unsafe_allow_html=True)
    st.markdown("<h4 style='margin:0 0 10px 0;'>📋 Kế hoạch thực hiện</h4>", unsafe_allow_html=True)
    m_plan = st.session_state.agent_state.get("master_plan", [])
    if m_plan:
        for step in m_plan:
            status_icon = "✅" if step["status"] == "completed" else "⏳"
            color = "#00ff88" if step["status"] == "completed" else "#888"
            st.markdown(f"<div style='font-size:12px; color:{color}; margin-bottom:4px;'>{status_icon} {step['task']}</div>", unsafe_allow_html=True)
    else:
    ...
    # Brain View
    st.markdown('<div class="glass-card">', unsafe_allow_html=True)
    st.markdown("<h4 style='margin:0 0 5px 0;'>🧠 Trạng thái</h4>", unsafe_allow_html=True)
    
    current_thought = st.session_state.agent_state["last_thought"]
    
    if auto_translate and current_thought:
        translated = translate_text(current_thought)
        st.markdown(f"<div style='font-size:13px; color:#00d4ff; border-left:2px solid #3e44fe; padding-left:8px; margin-bottom:4px;'>{translated}</div>", unsafe_allow_html=True)
        st.markdown(f"<p style='font-size:10px; color:#888; margin:0;'>{current_thought}</p>", unsafe_allow_html=True)
    else:
        st.markdown(f"<div style='font-size:13px; color:#00d4ff; border-left:2px solid #3e44fe; padding-left:8px;'>{current_thought}</div>", unsafe_allow_html=True)
    ...
    if st.session_state.agent_state["findings"]:
        for f in st.session_state.agent_state["findings"]:
            ...
            st.markdown(f'<div class="finding-item" style="padding: 6px; font-size: 12px; margin-bottom: 4px;">{f_text}</div>', unsafe_allow_html=True)
    else:
        st.markdown("<p style='font-size:12px; color:#888;'>Chưa có lỗi.</p>", unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)

    # BFS Progress View
    queue = st.session_state.agent_state.get("global_url_queue", [])
    plan = st.session_state.agent_state.get("current_page_plan", [])
    current_phase = st.session_state.agent_state.get("phase", "exploration")
    if queue:
        st.markdown('<div class="glass-card">', unsafe_allow_html=True)
        
        # Phase indicator
        is_sec = st.session_state.agent_state.get("test_security", False)
        is_ui = st.session_state.agent_state.get("test_ui", True)

        if current_phase == "security":
            phase_title = "Kiểm thử Bảo mật"
        elif current_phase == "login_action":
            phase_title = "Đăng nhập hệ thống (Bảo mật)" if is_sec else "Đăng nhập hệ thống"
        elif "exploration" in current_phase:
            if is_sec and not is_ui:
                phase_title = "Tìm kiếm lỗ hổng (Exploration)"
            else:
                phase_title = "Kiểm thử UI / Khám phá"
        elif current_phase == "completed":
            phase_title = "✅ Hoàn thành nhiệm vụ"
        else:
            phase_title = "Khởi tạo"
            
        st.markdown(f"<h4 style='margin:0 0 5px 0;'>🔍 {phase_title}</h4>", unsafe_allow_html=True)
        
        tested_pages = sum(1 for q in queue if q["status"] == "tested")
        total_pages = len(queue)
        st.progress(tested_pages / total_pages if total_pages > 0 else 0)
        st.markdown(f"<p style='font-size:11px; margin:2px 0;'>🌐 <b>Trang:</b> {tested_pages}/{total_pages} đã quét</p>", unsafe_allow_html=True)
        
        if current_phase == "security":
            sec_steps = st.session_state.agent_state.get("security_steps", 0)
            max_sec_steps = 15 # Đồng bộ với manager_node.py
            st.progress(min(sec_steps / max_sec_steps, 1.0))
            st.markdown(f"<p style='font-size:11px; margin:2px 0;'>🛡️ <b>Security Steps:</b> {sec_steps}/{max_sec_steps} hành động</p>", unsafe_allow_html=True)
        elif plan:
            st.markdown("<hr style='margin:8px 0; border:0; border-top:1px solid rgba(255,255,255,0.1);'>", unsafe_allow_html=True)
            tested_elements = sum(1 for p in plan if p["status"] in ["clicked", "skipped"])
            total_elements = len(plan)
            st.progress(tested_elements / total_elements if total_elements > 0 else 0)
            st.markdown(f"<p style='font-size:11px; margin:2px 0;'>📋 <b>UI Elements:</b> {tested_elements}/{total_elements} đã test</p>", unsafe_allow_html=True)
        
        # Scrollable small queue list (optional preview)
        with st.expander("Danh sách URL", expanded=False):
            queue_html = ""
            for q in queue:
                icon = {"pending": "⏳", "testing": "🔄", "tested": "✅"}.get(q["status"], "❓")
                url_short = q["url"].split("//")[-1]
                queue_html += f"<div style='font-size:10px; margin-bottom:2px;'>{icon} {url_short}</div>"
            st.markdown(queue_html, unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)

    # Timeline/History View
    st.markdown("<h4 style='margin:0 0 5px 0;'>📜 Nhật ký hành động</h4>", unsafe_allow_html=True)
    with st.container(height=280):
        for h in reversed(st.session_state.agent_state["history"]):
            if not isinstance(h, str):
                continue
            if "[VISION]" in h: label = '<span class="action-badge badge-vision">Vision</span>'
            elif "[MANAGER]" in h: label = '<span class="action-badge badge-manager">Think</span>'
            elif "[ACTION]" in h: label = '<span class="action-badge badge-action">Action</span>'
            else: label = ""
            
            clean_h = h.split("] ", 1)[-1] if "] " in h else h
            st.markdown(f"<div style='margin-bottom: 8px;'>{label} <span style='font-size: 13px;'>{clean_h}</span></div>", unsafe_allow_html=True)
...
"""
    ...
    # Extract detailed action steps (excluding AI thoughts)
    ...
    if action_steps:
        ...
        for step in action_steps:
            ...
            clean_step = clean_step.replace("--- Result: ", "")
            clean_step = clean_step.replace("---", "").strip()
            
            clean_step = clean_step.replace("Click success:", "Click thành công:")
            clean_step = clean_step.replace("Click failed:", "Click thất bại:")
            clean_step = clean_step.replace("Navigated to", "Đã chuyển hướng đến")
            clean_step = clean_step.replace("Navigation failed to", "Chuyển hướng thất bại đến")
            clean_step = clean_step.replace("Scrolled down.", "Đã cuộn xuống.")
            clean_step = clean_step.replace("Scrolled up.", "Đã cuộn lên.")
            clean_step = clean_step.replace("Position:", "Vị trí:")
            clean_step = clean_step.replace("More content below.", "Còn nội dung bên dưới.")
            clean_step = clean_step.replace("REACHED BOTTOM of page.", "Đã đến cuối trang.")
            clean_step = clean_step.replace("REACHED TOP of page.", "Đã đến đầu trang.")
            clean_step = clean_step.replace("Typed", "Đã nhập")
            clean_step = clean_step.replace("Type failed", "Nhập thất bại")
            clean_step = clean_step.replace("Form filled successfully:", "Điền biểu mẫu thành công:")
            clean_step = clean_step.replace("Form fill failed", "Điền biểu mẫu thất bại")
            clean_step = clean_step.replace("Completed testing page:", "Đã kiểm tra xong trang:")
            clean_step = clean_step.replace("External link detected", "Phát hiện liên kết ngoài")
            clean_step = clean_step.replace("auto navigated back to", "tự động quay lại")
            clean_step = clean_step.replace("Auto navigating back from", "Tự động quay lại từ")
            clean_step = clean_step.replace("New URL discovered:", "Phát hiện URL mới:")
            clean_step = clean_step.replace("Page plan created:", "Lên kế hoạch kiểm tra:")
...
"""
    ...
    from tools.report_tool import generate_excel_report, cleanup_reports
    safe_domain = base_url.replace("https://", "").replace("http://", "").split("/")[0] if base_url else "unknown"
    report_title = f"TEST {safe_domain}"
    excel_path = generate_excel_report(findings, title=report_title)
